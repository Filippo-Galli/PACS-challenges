...
function report_generation(){
    ...
    # Extract mean time each update
    mean_time_each_update=$(echo "$output" | grep -oP 'Mean time each update: \K[^ ]+(?: mus)?')
    # Remove ' mus'
    mean_time_each_update=${mean_time_each_update% mus}

    # Extract error to exact solution
    error_to_exact_solution=$(echo "$output" | grep -oP 'Error with exact solution: \K\d+\.\d+')

    printf "\t size: %3s Iteration count: %6s \t total_time %10s ms \t mean time one update %10s mus\t error: %8s\n" "$1" "$iter_count" "$total_time" "$mean_time_each_update" "$error_to_exact_solution" >> report.txt
}
...
printf "\nThis test are done with 8*pi^2*sin(2*pi*x)*sin(2*pi*y) as f and the correct solution is sin(2*pi*x)*sin(2*pi*y)\n";
printf "Firstly, it will ask for the max power of 2 to test (from 2 to 9) and then the max number of process of MPI to test.\n";
